...
// Handle image status update
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['updateStatus'])) {
    $imageId = $_POST['imageId'];
    $status = isset($_POST['status']) ? intval($_POST['status']) : 0;

    // Update status of the specific image in the database
    $updateSql = "UPDATE business_carousel SET status = :status WHERE bus_carou_id = :imageId";
    $updateStmt = $pdo->prepare($updateSql);
    $updateStmt->execute(array(':status' => $status, ':imageId' => $imageId));
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['fileUpload'])) {
    foreach ($_FILES['fileUpload']['name'] as $key => $name) {
        ...
        $sql = "INSERT INTO business_carousel (bus_id, images, status) VALUES (:bus_id, :images, 0)";
                                        ...
                                        <div class="card p-4">
                                            ...
                                            <div id="galleryImagesContainer" class="row">
                                                <?php foreach ($uploadedImages as $image) : ?>
                                                    <div class="col-3">
                                                        <div class="d-flex flex-column align-items-center">
                                                            ...
                                                            <form method="post" class="imageStatusForm">
                                                                <div class="mb-2 d-flex align-items-center">
                                                                    <label class="switch me-2">
                                                                        <input type="checkbox" class="imageToggleSwitch" data-id="<?php echo $image['bus_carou_id']; ?>" <?php echo $image['status'] == 1 ? 'checked' : ''; ?>>
                                                                        <span class="slider round"></span>
                                                                    </label>
                                                                    <label class="form-check-label">Public</label>
                                                                </div>
                                                            </form>
                                                            <img src="<?php echo $image['images']; ?>" class="img-fluid mb-2" alt="Gallery Image">
                                                            <div class="mt-2">
                                                                <!-- Edit button -->
                                                                <button class="btn btn-primary btn-sm" onclick="editImage('<?php echo $image['bus_carou_id']; ?>')">Edit</button>
                                                                <!-- Delete button -->
                                                                <button class="btn btn-danger btn-sm" onclick="deleteImage('<?php echo $image['bus_carou_id']; ?>')">Delete</button>
                                                            </div>
                                                        </div>
                                                    </div>
                                                <?php endforeach; ?>
                                                ...
                                                <div class="col-3" id="fullcontainer">
                                                    <form enctype="multipart/form-data" method="post" action="" type="submit" id="uploadForm">
                                                        <div id="upload-container">
                                                            <div id="upload-box">
                                                                <span id="plus-icon">+</span>
                                                                <input type="file" name="fileUpload[]" class="form-control" id="fileUpload" multiple>
                                                            </div>
                                                        </div>
                                                    </form>
...
document.addEventListener('DOMContentLoaded', (event) => {
    const imageToggleSwitches = document.querySelectorAll('.imageToggleSwitch');

    imageToggleSwitches.forEach((toggleSwitch) => {
        toggleSwitch.addEventListener('change', (event) => {
            ...
            .then(data => {
                if (data.success) {
                    Swal.fire({
                        title: status === 1 ? 'Image set to public' : 'Image set to private',
